...
SECRETS_FILE="$BDB_CHANNEL-$SECRET_FILE_ID-keys"
...
push_ecr_image(){
    ...
    docker build -t $ENVIRONMENT-$APP_NAME --build-arg BDB_ENV=$ENVIRONMENT --build-arg BDB_AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID --build-arg BDB_AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY --build-arg BDB_AWS_ACCOUNT_ID=$AWS_ACCOUNT_ID --build-arg BDB_SECRETS_FILE_ENV=$SECRETS_FILE --build-arg BDB_SECRETS_FILES_COUNT_ENV=$SECRETS_FILES_COUNT .
...
make_task_def(){
    ...
    NEW_CONTAINER_DEFINITIONS=$(echo "$TASK_TEMPLATE" | jq ".image = \"${AWS_ACCOUNT_ID}.dkr.ecr.us-east-1.amazonaws.com/${ENVIRONMENT}-${APP_NAME}:${VERSION}\""  )

	# task_def=$(printf "$NEW_CONTAINER_DEFINITIONS" $AWS_ACCOUNT_ID "latest")
    
    #task_def=$TASK_TEMPLATE
    task_def=$NEW_CONTAINER_DEFINITIONS

    echo "Template nuevo: $task_def"    
}
